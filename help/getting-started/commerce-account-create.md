...
>[!NOTE]
>
>In de sectie **[!UICONTROL Billing History]** op de [!DNL Commerce] -accountpagina worden alleen facturen weergegeven die zijn gemaakt voordat het factureringssysteem wordt bijgewerkt.
>
...
Een Adobe ID is vereist voor toegang tot uw [!DNL Commerce] -account. Als u een bestaande [!DNL Commerce] -account hebt, maar zich sinds augustus 2022 niet hebt aangemeld, moet u een Adobe ID maken tijdens het aanmeldingsproces. U moet deze stap voltooien voordat u zich kunt aanmelden bij uw account.
...
## Gedeelde toegang tot uw Commerce-account bieden
...
| Scenario | Wat gebeurt er? | Wat moet u doen met Adobe ID | Wat moet je doen voor MAGE ID / Commerce account | Resultaat |
| --- | --- | --- | --- | --- |
| Nieuw in Adobe en Commerce | Geen Adobe ID, geen MAGE-ID | Een Adobe ID maken wanneer hierom wordt gevraagd tijdens het aanmelden bij https://account.magento.com | Meld u opnieuw aan bij https://account.magento.com nadat Adobe ID is gemaakt | Er wordt een Commerce-account gemaakt en een MAGE-id gegenereerd en gekoppeld aan de Adobe ID |
| Heeft Adobe ID, maar nog geen MAGE-ID | Gebruikt alleen andere Adobe-producten | Aanmelden bij Adobe ID op https://account.magento.com | Als u zich voor het eerst aanmeldt, worden het Commerce-account en de MAGE-id gemaakt | MAGE-id wordt automatisch gemaakt en gekoppeld |
| Verouderde Commerce-klant met oude MAGE-ID | Heeft een historische MAGE-ID, maar geen Adobe ID | Een Adobe ID maken op https://account.adobe.com met dezelfde e-mail als de bestaande MAGE-id | Meld u aan bij https://account.magento.com met &quot;Aanmelden met Adobe ID&quot; | Bestaande MAGE-id is gekoppeld aan de nieuwe Adobe ID |
| Adobe ID- en MAGE-id bestaan maar zijn niet gekoppeld | E-mails voor Adobe ID en Commerce verschillen | De Adobe ID-e-mail uitlijnen zodat deze overeenkomt met de e-mail voor de Commerce-account (of andersom, afhankelijk van het eigendom) | Meld u aan via &quot;Aanmelden met Adobe ID&quot; op https://account.magento.com nadat de e-mailberichten overeenkomen | Adobe ID wordt de aanmeldingsnaam; de MAGE-id blijft de machtigings-id |
| Heeft Adobe ID maar geen MAGE-ID | Nooit aangemeld bij https://account.magento.com | De bestaande Adobe ID gebruiken | Aanmelden bij https://account.magento.com voor het eerst | Met deze eerste aanmelding wordt een MAGE-id gegenereerd en gekoppeld |
| Gebruikt alleen aanmelding voor cloudproject (accounts.magento.cloud) | Kan cloudproject openen, maar geen klassiek Commerce-account | Doorgaan met Adobe ID for cloud-project | Meld u ook aan bij https://account.magento.com met dezelfde Adobe ID als u Marketplace/licenties nodig hebt | Er wordt een klassieke Commerce-account (met MAGE-id) gemaakt en gekoppeld |

Als een gebruiker van mening is dat hij of zij Commerce-rechten moet hebben maar geen MAGE-id ziet, is de volgende standaardstap het aanmelden via https://account.magento.com zodat een Commerce-account en MAGE-id kunnen worden gemaakt of correct kunnen worden gekoppeld.
