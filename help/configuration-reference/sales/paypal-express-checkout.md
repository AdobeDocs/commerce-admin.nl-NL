...
| [!UICONTROL API Authentication Methods] | Website | Bepaalt welke methode wordt gebruikt voor API-verificatie. Opties: <br/>**`API Signature`**- geeft het veld _[!UICONTROL API Signature]_in het formulier weer.<br/>**`API Certificate`**- Hiermee geeft u het veld_[!UICONTROL API Certificate]_ in het formulier weer. |
