...
## Enable batch sitemap generation for large catalogi

Voor winkels met grote catalogi gebruikt u de volgende alternatieve snijtaak om het genereren van batch sitemap in te schakelen. Deze aanpak verwerkt gegevens in kleinere stappen, waardoor het risico op uitputting van het PHP-geheugen aanzienlijk wordt verminderd en waardoor sitemap-generatie met succes wordt voltooid, zelfs voor sites met uitgebreide productgegevens.

In `app/code/Magento/Sitemap/etc/config.xml` vervangen:
